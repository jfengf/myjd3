...
async function help() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${$.shopId}%22,%22itemToken%22:%22${
    $.helpCode
  }%22,%22token%22:%22${$.tokenId}%22%7D&appid=xdz&functionId=mcxhd_starmall_doTask&t=${Date.now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    Origin: `https://h5.m.jd.com`,
    Cookie: $.cookie,
...
async function drawAward() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${$.shopId}%22,%22token%22:%22${
    $.tokenId
