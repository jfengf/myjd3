...
async function jdFruit() {
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      console.log(
        `初始化农场数据异常, 请登录京东 app查看农场0元水果功能是否正常,农场初始化数据: ${JSON.stringify($.farmInfo)}`
      );
...
async function predictionFruit() {
  ...
  // 预测n天后水果课可兑换功能
  let waterTotalT =
    ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy - $.farmInfo.farmUserPro.totalEnergy) /
    10; //一共还需浇多少次水

  let waterD = Math.ceil(waterTotalT / waterEveryDayT);

  message += `【预测】${waterD === 1 ? "明天" : waterD === 2 ? "后天" : waterD + "天之后"}(${timeFormat(
    24 * 60 * 60 * 1000 * waterD + Date.now()
  )}日)可兑换水果🍉`;
}
//浇水十次
async function doTenWater() {
  ...
  if ($.farmTask.totalWaterTaskInit.totalWaterTaskTimes < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    ...
    for (
      ;
      waterCount <
      $.farmTask.totalWaterTaskInit.totalWaterTaskLimit - $.farmTask.totalWaterTaskInit.totalWaterTaskTimes;
      waterCount++
    ) {
...
async function getTenWaterAward() {
  ...
  if (
    ...
    $.farmTask.totalWaterTaskInit.totalWaterTaskTimes >= $.farmTask.totalWaterTaskInit.totalWaterTaskLimit
  ) {
...
async function turntableFarm() {
  ...
  if ($.initForTurntableFarmRes.code === "0") {
    ...
    if (!timingGotStatus) {
      console.log(
        `是否到了领取免费赠送的抽奖机会----${sysTime > timingLastSysTime + 60 * 60 * timingIntervalHours * 1000}`
      );
  ...
  if (executeWaterRain) {
    ...
    if ($.farmTask.waterRainInit.lastTime) {
      if (Date.now() < $.farmTask.waterRainInit.lastTime + 3 * 60 * 60 * 1000) {
        ...
        console.log(
          `\`【第${$.farmTask.waterRainInit.winTimes + 1}次水滴雨】未到时间，请${new Date(
...
function readShareCode() {
  return new Promise(async (resolve) => {
    $.get(
      { url: `http://share.turinglabs.net/api/v3/farm/query/${randomCount}/`, timeout: 10000 },
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
