...
!(async () => {
  ...
  process.env.JD_BEAN_SIGN_NOTIFY_SIMPLE = process.env.JD_BEAN_SIGN_NOTIFY_SIMPLE
...
async function execSign() {
  ...
  try {
    ...
    if (BarkContent) {
      ...
      if (
        !process.env.JD_BEAN_SIGN_NOTIFY_SIMPLE ||
        (process.env.JD_BEAN_SIGN_NOTIFY_SIMPLE && process.env.JD_BEAN_SIGN_NOTIFY_SIMPLE !== "true")
      ) {
    ...
    console.log(
      `\n\n*****************${new Date(new Date().getTime()).toLocaleString("zh", { hour12: false })} 京东账号${
        $.index
      } ${$.nickName || $.UserName} ${$.name}完成*******************\n\n`
    );
  } catch (e) {
